...
<a href="<c:url value="/board/write?pageSize=${ph.sc.pageSize}"/>">게시글 작성</a>
...
<c:if test="${ph.showPrev}"><a href="<c:url value="/board/list?page=${ph.beginPage-1}&pageSize=${ph.sc.pageSize}"/>">&lt;&lt;</a></c:if>
...
<c:choose>
  ...
  <c:when test="${not empty ph.sc.keyword}">
    <a href="<c:url value="/board/list?option=${ph.sc.option}keyword=${ph.sc.keyword}&page=${i}&pageSize=${ph.sc.pageSize}"/>">${i}</a>
  </c:when>
</c:choose>
</c:forEach>

<c:if test="${ph.showNext}"><a href="<c:url value="/board/list?page=${ph.endPage+1}&pageSize=${ph.sc.pageSize}"/>">&gt;&gt;</a></c:if>
